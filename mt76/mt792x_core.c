...
static const struct ieee80211_iface_limit if_limits_chanctx[] = {
    {.max = 2,
     .types = BIT(NL80211_IFTYPE_STATION) | BIT(NL80211_IFTYPE_P2P_CLIENT)},
    {.max = 1, .types = BIT(NL80211_IFTYPE_AP) | BIT(NL80211_IFTYPE_P2P_GO)}};
...
void mt7902_mt792x_tx(struct ieee80211_hw *hw,
...
void mt7902_mt792x_tx_worker(struct mt7902_mt76_worker *w) {
  struct mt7902_mt792x_dev *dev =
      container_of(w, struct mt7902_mt792x_dev, mt76.tx_worker);
...
void mt7902_mt792x_flush(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
  ...
  wait_event_timeout(dev->mt76.tx_wait, !mt7902_mt76_has_tx_pending(&dev->mphy),
                     HZ / 2);
}
...
void mt7902_mt792x_get_et_strings(struct ieee80211_hw *hw,
                                  struct ieee80211_vif *vif, u32 sset,
                                  u8 *data) {
  if (sset != ETH_SS_STATS)
    return;

  memcpy(data, mt7902_mt792x_gstrings_stats,
         sizeof(mt7902_mt792x_gstrings_stats));
...
int mt7902_mt792x_get_et_sset_count(struct ieee80211_hw *hw,
...
static void mt7902_mt792x_ethtool_worker(void *wi_data,
...
void mt7902_mt792x_get_et_stats(struct ieee80211_hw *hw,
                                struct ieee80211_vif *vif,
...
void mt7902_mt792x_set_coverage_class(struct ieee80211_hw *hw,
                                      s16 coverage_class) {
  struct mt7902_mt792x_phy *phy = mt7902_mt792x_hw_phy(hw);
  struct mt7902_mt792x_dev *dev = phy->dev;
...
int mt7902_mt792x_init_wiphy(struct ieee80211_hw *hw) {
  ...
  wiphy->flags &=
      ~(WIPHY_FLAG_IBSS_RSN | WIPHY_FLAG_4ADDR_AP | WIPHY_FLAG_4ADDR_STATION);
  ...
  wiphy->flags |= WIPHY_FLAG_HAS_CHANNEL_SWITCH | WIPHY_FLAG_SPLIT_SCAN_6GHZ;
...
static u8 mt7902_mt792x_get_offload_capability(struct device *dev,
                                               const char *fw_wm) {
...
mt7902_mt792x_get_mac80211_ops(struct device *dev,
  ...
  ops =
      devm_kmemdup(dev, mac80211_ops, sizeof(struct ieee80211_ops), GFP_KERNEL);
  if (!ops)
    return NULL;
...
int mt7902_mt792xe_mcu_drv_pmctrl(struct mt7902_mt792x_dev *dev) {
  ...
  pm->stats.doze_time += pm->stats.last_wake_event - pm->stats.last_doze_event;
out:
  return err;
}
...
int mt7902_mt792xe_mcu_fw_pmctrl(struct mt7902_mt792x_dev *dev) {
  ...
  pm->stats.awake_time += pm->stats.last_doze_event - pm->stats.last_wake_event;
...
int mt7902_mt792x_load_firmware(struct mt7902_mt792x_dev *dev) {
  int ret;
  const char *patch_names[] = {mt7902_mt792x_patch_name(dev), MT7922_ROM_PATCH,
                               MT7921_ROM_PATCH, MT7925_ROM_PATCH, NULL};
  const char *ram_names[] = {mt7902_mt792x_ram_name(dev), MT7922_FIRMWARE_WM,
                             MT7921_FIRMWARE_WM, MT7925_FIRMWARE_WM, NULL};
  int i;

  /* Try a set of possible patch firmware names until one succeeds. */
  ret = -ENOENT;
  for (i = 0; patch_names[i]; i++) {
    const char *p = patch_names[i];
    if (!p)
      break;
    dev_info(dev->mt76.dev, "attempting to load ROM patch: %s\n", p);
    ret = mt7902_mt76_connac2_load_patch(&dev->mt76, p);
    if (!ret) {
      dev_info(dev->mt76.dev, "loaded ROM patch: %s\n", p);
      break;
    }
    dev_warn(dev->mt76.dev, "failed to load ROM patch %s: %d\n", p, ret);
  }
  if (ret)
    return ret;
  ...
  /* Try ram firmware names similarly */
  ret = -ENOENT;
  for (i = 0; ram_names[i]; i++) {
    const char *r = ram_names[i];
    if (!r)
      break;
    dev_info(dev->mt76.dev, "attempting to load RAM firmware: %s\n", r);
    ret = mt7902_mt76_connac2_load_ram(&dev->mt76, r, NULL);
    if (!ret) {
      dev_info(dev->mt76.dev, "loaded RAM firmware: %s\n", r);
      break;
    }
    dev_warn(dev->mt76.dev, "failed to load RAM firmware %s: %d\n", r, ret);
  }
  if (ret)
    return ret;
