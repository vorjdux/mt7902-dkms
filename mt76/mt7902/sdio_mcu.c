...
static int mt7902s_mcu_send_message(struct mt7902_mt76_dev *mdev,
                                    struct sk_buff *skb, int cmd, int *seq) {
  struct mt7902_mt7902_mt792x_dev *dev =
      container_of(mdev, struct mt7902_mt7902_mt792x_dev, mt76);
   ...
   * blocked by mt7902_mt7902_mt792x_mac_work that is excuting register access
   * via sdio bus.
   */
  if (dev->fw_assert)
    return -EBUSY;
  ...
  mt7902_mt7902_mt792x_skb_add_usb_sdio_hdr(dev, skb, type);
...
static u32 mt7902s_read_rm3r(struct mt7902_mt7902_mt792x_dev *dev) {
  struct mt7902_mt76_sdio *sdio = &dev->mt76.sdio;

  return sdio_readl(sdio->func, MCR_D2HRM3R, NULL);
}

static u32 mt7902s_clear_rm3r_drv_own(struct mt7902_mt7902_mt792x_dev *dev) {
  struct mt7902_mt76_sdio *sdio = &dev->mt76.sdio;
  u32 val;

  val = sdio_readl(sdio->func, MCR_D2HRM3R, NULL);
  if (val)
    sdio_writel(sdio->func, H2D_SW_INT_CLEAR_MAILBOX_ACK, MCR_WSICR, NULL);

  return val;
}

int mt7902s_mcu_init(struct mt7902_mt7902_mt792x_dev *dev) {
  static const struct mt7902_mt76_mcu_ops mt7902s_mcu_ops = {
      .headroom = MT_SDIO_HDR_SIZE + sizeof(struct mt7902_mt76_connac2_mcu_txd),
...
int mt7902s_mcu_drv_pmctrl(struct mt7902_mt7902_mt792x_dev *dev) {
  ...
  if (!err && test_bit(MT76_STATE_MCU_RUNNING, &dev->mphy.state))
    err = readx_poll_timeout(mt7902s_read_rm3r, dev, status,
                             status & D2HRM3R_IS_DRIVER_OWN, 2000, 1000000);
...
int mt7902s_mcu_fw_pmctrl(struct mt7902_mt7902_mt792x_dev *dev) {
  struct sdio_func *func = dev->mt76.sdio.func;
  struct mt7902_mt76_phy *mphy = &dev->mt76.phy;
  struct mt7902_mt76_connac_pm *pm = &dev->pm;
  u32 status;
  ...
  if (test_bit(MT76_STATE_MCU_RUNNING, &dev->mphy.state)) {
    err = readx_poll_timeout(mt7902s_clear_rm3r_drv_own, dev, status,
                             !(status & D2HRM3R_IS_DRIVER_OWN), 2000, 1000000);
    if (err < 0) {
