...
static void mt7902_init_he_caps(struct mt7902_mt792x_phy *phy,
  ...
  case NL80211_IFTYPE_AP:
    ...
    he_cap_elem->phy_cap_info[6] |= IEEE80211_HE_PHY_CAP6_PARTIAL_BW_EXT_RANGE |
...
static void mt7902_init_eht_caps(struct mt7902_mt792x_phy *phy,
                                 enum nl80211_band band,
                                 struct ieee80211_sband_iftype_data *data) {
  ...
  eht_cap_elem->mac_cap_info[0] = IEEE80211_EHT_MAC_CAP0_EPCS_PRIO_ACCESS |
                                  IEEE80211_EHT_MAC_CAP0_OM_CONTROL;

  eht_cap_elem->phy_cap_info[0] = IEEE80211_EHT_PHY_CAP0_NDP_4_EHT_LFT_32_GI |
  ...
  eht_cap_elem->phy_cap_info[1] =
      ...
      u8_encode_bits(sts - 1, IEEE80211_EHT_PHY_CAP1_BEAMFORMEE_SS_160MHZ_MASK);
  ...
  eht_cap_elem->phy_cap_info[4] = u8_encode_bits(
      min_t(int, sts - 1, 2), IEEE80211_EHT_PHY_CAP4_MAX_NC_MASK);
  ...
  val = width == NL80211_CHAN_WIDTH_160  ? 0x7
        : width == NL80211_CHAN_WIDTH_80 ? 0x3
                                         : 0x1;
...
int __mt7902_start(struct mt7902_mt792x_phy *phy) {
  ...
  ieee80211_queue_delayed_work(mphy->hw, &mphy->mac_work, MT792x_WATCHDOG_TIME);
...
static int mt7902_add_interface(struct ieee80211_hw *hw,
  ...
  mt7902_mac_wtbl_update(dev, idx, MT_WTBL_UPDATE_ADM_COUNT_CLEAR);
...
static void mt7902_roc_iter(void *priv, u8 *mac, struct ieee80211_vif *vif) {
  struct mt7902_mt792x_vif *mvif = (struct mt7902_mt792x_vif *)vif->drv_priv;
...
void mt7902_roc_work(struct work_struct *work) {
  ...
  ieee80211_iterate_active_interfaces(
      phy->mt76->hw, IEEE80211_IFACE_ITER_RESUME_ALL, mt7902_roc_iter, phy);
  mt7902_mt792x_mutex_release(phy->dev);
  ieee80211_remain_on_channel_expired(phy->mt76->hw);
}

static int mt7902_abort_roc(struct mt7902_mt792x_phy *phy,
...
static int mt7902_set_roc(struct mt7902_mt792x_phy *phy,
  ...
  err = mt7902_mcu_set_roc(phy, vif, chan, duration, type, ++phy->roc_token_id);
  if (err < 0) {
...
static int mt7902_set_key(struct ieee80211_hw *hw, enum set_key_cmd cmd,
  ...
  mt7902_mt76_wcid_key_setup(&dev->mt76, wcid, cmd == SET_KEY ? key : NULL);

  err = mt7902_mcu_add_key(&dev->mt76, vif, &msta->bip, key,
                           MCU_UNI_CMD(STA_REC_UPDATE), &msta->wcid, cmd);
  ...
  if (key->cipher == WLAN_CIPHER_SUITE_WEP104 ||
    ...
    err = mt7902_mcu_add_key(&dev->mt76, vif, &mvif->wep_sta->bip, key,
...
static void mt7902_pm_interface_iter(void *priv, u8 *mac,
                                     struct ieee80211_vif *vif) {
...
static void mt7902_sniffer_interface_iter(void *priv, u8 *mac,
                                          struct ieee80211_vif *vif) {
  struct mt7902_mt792x_dev *dev = priv;
  struct ieee80211_hw *hw = mt7902_mt76_hw(dev);
...
#define MT7925_FILTER(_fif, _type)                                             \
  do {                                                                         \
    if (*total_flags & (_fif))                                                 \
...
static u8 mt7902_get_rates_table(struct ieee80211_hw *hw,
                                 struct ieee80211_vif *vif, bool beacon,
                                 bool mcast) {
...
static void mt7902_bss_info_changed(struct ieee80211_hw *hw,
  ...
  if (changed & BSS_CHANGED_MCAST_RATE)
    mvif->mcast_rates_idx = mt7902_get_rates_table(hw, vif, false, true);

  if (changed & BSS_CHANGED_BASIC_RATES)
    mvif->basic_rates_idx = mt7902_get_rates_table(hw, vif, false, false);

  if (changed & (BSS_CHANGED_BEACON | BSS_CHANGED_BEACON_ENABLED)) {
    mvif->beacon_rates_idx = mt7902_get_rates_table(hw, vif, true, false);

    mt7902_mcu_uni_add_beacon_offload(dev, hw, vif, info->enable_beacon);
  }
  ...
  if (changed & BSS_CHANGED_ASSOC) {
    mt7902_mcu_sta_update(dev, NULL, vif, true, MT76_STA_INFO_STATE_ASSOC);
...
int mt7902_mac_sta_add(struct mt7902_mt76_dev *mdev, struct ieee80211_vif *vif,
  ...
  struct mt7902_mt792x_dev *dev =
      container_of(mdev, struct mt7902_mt792x_dev, mt76);
  ...
  if (vif->type == NL80211_IFTYPE_STATION && !sta->tdls)
    mt7902_mcu_add_bss_info(&dev->phy, mvif->mt76.ctx, vif, sta, false);

  ret = mt7902_mcu_sta_update(dev, sta, vif, true, MT76_STA_INFO_STATE_NONE);
  if (ret)
    return ret;
...
void mt7902_mac_sta_assoc(struct mt7902_mt76_dev *mdev,
                          struct ieee80211_vif *vif,
                          struct ieee80211_sta *sta) {
  struct mt7902_mt792x_dev *dev =
      container_of(mdev, struct mt7902_mt792x_dev, mt76);
  ...
  if (vif->type == NL80211_IFTYPE_STATION && !sta->tdls)
    mt7902_mcu_add_bss_info(&dev->phy, mvif->mt76.ctx, vif, sta, true);

  ewma_avg_signal_init(&msta->avg_ack_signal);

  mt7902_mac_wtbl_update(dev, msta->wcid.idx, MT_WTBL_UPDATE_ADM_COUNT_CLEAR);
...
void mt7902_mac_sta_remove(struct mt7902_mt76_dev *mdev,
                           struct ieee80211_vif *vif,
                           struct ieee80211_sta *sta) {
  struct mt7902_mt792x_dev *dev =
      container_of(mdev, struct mt7902_mt792x_dev, mt76);
...
static int mt7902_ampdu_action(struct ieee80211_hw *hw,
                               struct ieee80211_vif *vif,
...
static int mt7902_hw_scan(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
...
static void mt7902_cancel_hw_scan(struct ieee80211_hw *hw,
                                  struct ieee80211_vif *vif) {
  struct mt7902_mt792x_dev *dev = mt7902_mt792x_hw_dev(hw);
  struct mt7902_mt76_phy *mphy = hw->priv;
...
static int mt7902_start_sched_scan(struct ieee80211_hw *hw,
                                   struct ieee80211_vif *vif,
...
static int mt7902_stop_sched_scan(struct ieee80211_hw *hw,
                                  struct ieee80211_vif *vif) {
  struct mt7902_mt792x_dev *dev = mt7902_mt792x_hw_dev(hw);
  struct mt7902_mt76_phy *mphy = hw->priv;
  int err;
...
static int mt7902_set_antenna(struct ieee80211_hw *hw, u32 tx_ant, u32 rx_ant) {
  struct mt7902_mt792x_dev *dev = mt7902_mt792x_hw_dev(hw);
  struct mt7902_mt792x_phy *phy = mt7902_mt792x_hw_phy(hw);
...
static int mt7902_suspend(struct ieee80211_hw *hw,
  ...
  ieee80211_iterate_active_interfaces(hw, IEEE80211_IFACE_ITER_RESUME_ALL,
                                      mt7902_mcu_set_suspend_iter, &dev->mphy);
...
static int mt7902_resume(struct ieee80211_hw *hw) {
  ...
  ieee80211_queue_delayed_work(hw, &phy->mt76->mac_work, MT792x_WATCHDOG_TIME);
...
static void mt7902_ipv6_addr_change(struct ieee80211_hw *hw,
  ...
  skb = __mt7902_mt76_mcu_msg_alloc(&dev->mt76, NULL, sizeof(req_hdr), 0,
                                    GFP_ATOMIC);
  if (!skb)
    return;
...
static int mt7902_set_sar_specs(struct ieee80211_hw *hw,
  ...
  mt7902_mt792x_mutex_acquire(dev);
  err = mt7902_mcu_set_clc(dev, dev->mt76.alpha2, dev->country_ie_env);
  if (err < 0)
    goto out;
...
static void mt7902_channel_switch_beacon(struct ieee80211_hw *hw,
                                         struct ieee80211_vif *vif,
...
static int mt7902_conf_tx(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
...
static int mt7902_start_ap(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
  ...
  err = mt7902_mcu_add_bss_info(&dev->phy, mvif->mt76.ctx, vif, NULL, true);
  if (err)
    goto out;
  ...
  err = mt7902_mcu_sta_update(dev, NULL, vif, true, MT76_STA_INFO_STATE_NONE);
out:
  mt7902_mt792x_mutex_release(dev);
...
static void mt7902_stop_ap(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
                           struct ieee80211_bss_conf *link_conf) {
  struct mt7902_mt792x_vif *mvif = (struct mt7902_mt792x_vif *)vif->drv_priv;
  struct mt7902_mt792x_dev *dev = mt7902_mt792x_hw_dev(hw);
  int err;
  ...
  mt7902_mcu_add_bss_info(&dev->phy, mvif->mt76.ctx, vif, NULL, false);

out:
  mt7902_mt792x_mutex_release(dev);
}

static int mt7902_add_chanctx(struct ieee80211_hw *hw,
                              struct ieee80211_chanctx_conf *ctx) {
  return 0;
}

static void mt7902_remove_chanctx(struct ieee80211_hw *hw,
                                  struct ieee80211_chanctx_conf *ctx) {}

static void mt7902_ctx_iter(void *priv, u8 *mac, struct ieee80211_vif *vif) {
  struct mt7902_mt792x_vif *mvif = (struct mt7902_mt792x_vif *)vif->drv_priv;
...
static void mt7902_change_chanctx(struct ieee80211_hw *hw,
                                  struct ieee80211_chanctx_conf *ctx,
                                  u32 changed) {
  struct mt7902_mt792x_phy *phy = mt7902_mt792x_hw_phy(hw);
  ...
  mt7902_mt792x_mutex_acquire(phy->dev);
  ieee80211_iterate_active_interfaces(
      phy->mt76->hw, IEEE80211_IFACE_ITER_ACTIVE, mt7902_ctx_iter, ctx);
  mt7902_mt792x_mutex_release(phy->dev);
}
