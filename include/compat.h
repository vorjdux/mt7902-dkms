...
#define MT7902_SURVEY_SIGNATURE(hw, idx, survey)                               \
  mt7902_get_survey_compat(struct ieee80211_hw *hw, int idx,                   \
                           struct survey_info *survey, u32 flags)
#else
#define MT7902_SURVEY_SIGNATURE(hw, idx, survey)                               \
  mt7902_get_survey(struct ieee80211_hw *hw, int idx,                          \
...
#define MT7902_CONFIG_SIGNATURE(hw, changed)                                   \
  mt7902_config_compat(struct ieee80211_hw *hw, u32 changed,                   \
...
#define MT7902_BSS_INFO_CHANGED_SIGNATURE(hw, vif, info, changed)              \
  mt7902_bss_info_changed_compat(                                              \
...
/* Increased retries to handle slower/laggy firmware responses observed
 * on some kernels and hardware. This is a low-risk increase and kept
 * centralized here so it can be tuned without touching driver logic.
 */
#define MT7902_PATCH_SEM_RETRIES 8
#else
 ...
 */
static inline int mt7902_patch_sem_ctrl_with_retry(struct mt7902_mt76_dev *dev,
                                                   bool get) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 14, 0)
  ...
  do {
    ...
    /* Backoff between retries to give firmware/MCU time to settle. */
    msleep(150);
  } while (--retries > 0);
